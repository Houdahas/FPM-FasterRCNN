...
def draw_bboxes_with_labels(img, bboxes, label_indices, probs, labels):
    ...
    """
    colors = tf.random.uniform((len(labels), 1), maxval=800, dtype=tf.int32)
    #colors = [1,2,0]
    ...
    for index, bbox in enumerate(bboxes):
        ...
        color = int(colors[label_index])
        label_text = "{0} {1:0.2f}".format(labels[label_index], probs[index])
...
def draw_predictions(dataset, pred_bboxes, pred_labels, pred_scores, labels, batch_size):
     for batch_id, image_data in enumerate(dataset):
        ...
        for i, img in enumerate(imgs):
            img = np.reshape(img[0,:,:,0],(896,896,1))
